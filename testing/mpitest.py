...
if __name__ == '__main__':
    ...
    logging.debug(f'rank: {rank}, size: {size}')
    ...
    if rank_sum == sum(range(rank + 1)):
        logging.info(f'Rank {comm_world.Get_rank()}: Rank sum equal! Exiting as normal...')
    else:
        logging.info(f'Rank {comm_world.Get_rank()}: Rank sum unequal! Exiting with error code 1...')
        sys.exit(1)
