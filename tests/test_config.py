...
@pytest.mark.parametrize(
        ...
        [
            ...
            (SonyBayerConfig.device_voxels_simulation_mesh_lateral, 1 + int(2.04 / 0.017)),
            (SonyBayerConfig.device_voxels_simulation_mesh_vertical, 2 + int((0.408 * 5) /0.017)),
            (SonyBayerConfig.device_vertical_maximum_um, 0.408 * 5),
            (SonyBayerConfig.device_size_lateral_bordered_um, 2.04),
            (SonyBayerConfig.device_voxels_simulation_mesh_lateral_bordered, 1 + int(2.04 / 0.017)),
            ...
            (SonyBayerConfig.fdtd_region_size_vertical_um, 2 * (0.085 * 15) + (0.408 * 5) + (0.051 * 30)),
            (SonyBayerConfig.fdtd_region_size_lateral_um, 2 * (0.051 * 10) + 2.04),
            (SonyBayerConfig.fdtd_region_maximum_vertical_um, 2.04 + (0.085 * 15)),
            (SonyBayerConfig.fdtd_region_minimum_vertical_um, -1 * 0.051 * 30 - (0.085 * 15)),

            # Surrounding Properties
            (SonyBayerConfig.pec_aperture_thickness_um, 3 * 0.017),
            (SonyBayerConfig.sidewall_x_positions_um, [(2.04 + 0.085) / 2, 0, -(2.04 + 0.085)/2, 0]),
            (SonyBayerConfig.sidewall_y_positions_um, [0, (2.04 + 0.085) / 2, 0, -(2.04 + 0.085)/2]),
            (SonyBayerConfig.sidewall_xspan_positions_um, [0.085, 2.04 + 2 * 0.085, 0.085, 2.04 + 2 * 0.085]),
            (SonyBayerConfig.sidewall_yspan_positions_um, [2.04 + 0.085 * 2, 0.085, 2.04 + 2 * 0.085, 0.085]),
            ...
            (SonyBayerConfig.max_intensity_by_wavelength, (2.04 ** 4) / ((0.051 * 30)**2 * np.power(np.linspace(0.375, 0.725, 3 * 20), 2))),
            ...
            (SonyBayerConfig.pdaf_lambda_values_um, np.linspace(0.375, 0.375 + (0.725 - 0.375) / 3, 3 * 20)),
            (SonyBayerConfig.pdaf_max_intensity_by_wavelength, (2.04 **4) / ((0.051 * 30)**2 * np.power(np.linspace(0.375, 0.375 + (0.725 - 0.375) / 3, 3 * 20), 2))),

            # Forward Properties
            (SonyBayerConfig.lateral_aperture_um, 1.1 * 2.04),
            (SonyBayerConfig.src_maximum_vertical_um, 0.408 * 5 + (0.085 * 15 * 2 / 3)),
            (SonyBayerConfig.src_minimum_vertical_um, -(0.051 * 30) - 0.5 * (0.085 * 15)),
            (SonyBayerConfig.mid_lambda_um, (0.375 + 0.725) / 2),
            (SonyBayerConfig.source_angle_theta_rad, np.arcsin(np.sin(0)  * 1.0 / 1.5)),
            (SonyBayerConfig.source_angle_theta_deg, np.arcsin(np.sin(0)  * 1.0 / 1.5) * 180 / np.pi),
        ]
)
