...
bayer_filter_region = np.zeros( ( device_voxels_lateral, device_voxels_lateral, device_voxels_vertical, 3 ) )
...
for epoch in range(start_epoch, num_epochs):
	...
	for iteration in range(start_iter, num_iterations_per_epoch):
		...
		for xy_idx in range(0, 2):
			...
			quadrant_transmission_data[xy_names[xy_idx]] = [ None for idx in range( 0, num_focal_spots ) ]
		...
		for focal_idx in range(0, num_focal_spots):
			...
			for polarization_idx in range(0, len(polarizations)):
				...
				get_quad_transmission_data = quadrant_transmission_data[polarizations[polarization_idx]]
				...
				for spectral_idx in range(0, total_weighting.shape[0]):
					...
					compute_transmission_fom += weight_spectral_rejection * get_quad_transmission_data[focal_idx][spectral_focal_plane_map[focal_idx][0] + spectral_idx] / one_over_weight_focal_plane_map_performance_weighting
					...
					transmission_by_wavelength[ spectral_focal_plane_map[focal_idx][0] + spectral_idx ] += weight_spectral_rejection * get_quad_transmission_data[focal_idx][spectral_focal_plane_map[focal_idx][0] + spectral_idx] / one_over_weight_focal_plane_map_performance_weighting
		...
		if do_rejection:
			for idx in range( 0, len( figure_of_merit_per_focal_spot ) ):
				figure_of_merit_per_focal_spot[ idx ] = softplus( figure_of_merit_per_focal_spot[ idx ] )
			for idx in range( 0, len( transmission_per_quadrant ) ):
				transmission_per_quadrant[ idx ] = softplus( transmission_per_quadrant[ idx ] )

			for idx in range( 0, len( intensity_fom_by_wavelength ) ):
				intensity_fom_by_wavelength[ idx ] = softplus( intensity_fom_by_wavelength[ idx ] )
			for idx in range( 0, len( transmission_by_wavelength ) ):
				transmission_by_wavelength[ idx ] = softplus( transmission_by_wavelength[ idx ] )
		...
		if weight_by_quadrant_transmission:
			performance_weighting = (2. / num_focal_spots) - transmission_per_quadrant**2 / np.sum(transmission_per_quadrant**2)

		if weight_by_individual_wavelength:
			performance_weighting = (2. / num_design_frequency_points) - intensity_fom_by_wavelength**2 / np.sum( intensity_fom_by_wavelength**2 )
			if weight_by_quadrant_transmission:
				performance_weighting = (2. / num_design_frequency_points) - transmission_by_wavelength**2 / np.sum( transmission_by_wavelength**2 )
				...
				for xy_idx in range(0, 2):
					...
					for spectral_idx in range(0, len( source_weight ) ):
						if weight_by_individual_wavelength:
							...
							if do_rejection:
								if weight_by_quadrant_transmission:
									gradient_performance_weight *= softplus_prime( transmission_by_wavelength[ adj_src_idx ] )
								else:
									gradient_performance_weight *= softplus_prime( intensity_fom_by_wavelength[ adj_src_idx ] )
