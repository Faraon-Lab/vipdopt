...
def disable_all_sources():
	...
	if add_pdaf:
		for xy_idx in range(0, 2):
			fdtd_hook.select( pdaf_sources[xy_idx]['name'] )
			fdtd_hook.set( 'enabled', 0 )

		for xy_idx in range(0, 2):
			fdtd_hook.select( pdaf_adjoint_sources[xy_idx]['name'] )
			fdtd_hook.set( 'enabled', 0 )
...
if start_from_step == 0:
	...
	if add_pdaf:
		...
		for xy_idx in range(0, 2):
			pdaf_adjoint_source = fdtd_hook.adddipole()
...
# pdaf_gaussian_defocus_um = -4
pdaf_gaussian_defocus_um = 6
...
if restart or evaluate:
	...
	step_size_evolution = np.load(projects_directory_location + "/step_size_evolution.npy")
	average_design_variable_change_evolution = np.load(projects_directory_location + "/average_design_change_evolution.npy")
	max_design_variable_change_evolution = np.load(projects_directory_location + "/max_design_change_evolution.npy")
	figure_of_merit_evolution = np.load(projects_directory_location + "/figure_of_merit.npy")
	figure_of_merit_by_wl_evolution = np.load(projects_directory_location + "/figure_of_merit_by_wl.npy" )
	transmission_by_focal_spot_evolution = np.load(projects_directory_location + "/transmission_by_focal_spot.npy")
	transmission_by_wl_evolution = np.load(projects_directory_location + "/transmission_by_wl.npy")
...
if evaluate:
	...
	# transmission_monitor_device_input = fdtd_hook.addpower()
	# transmission_monitor_device_input['name'] = 'transmission_monitor_device_input'
	# transmission_monitor_device_input['monitor type'] = '2D Z-normal'
	# transmission_monitor_device_input['x'] = 0 * 1e-6
	# transmission_monitor_device_input['x span'] = device_size_lateral_um * 1e-6
	# transmission_monitor_device_input['y'] = 0 * 1e-6
	# transmission_monitor_device_input['y span'] = device_size_lateral_um * 1e-6
	# transmission_monitor_device_input['z'] = device_vertical_maximum_um * 1e-6
	# transmission_monitor_device_input['override global monitor settings'] = 1
	# transmission_monitor_device_input['use wavelength spacing'] = 1
	# transmission_monitor_device_input['use source limits'] = 1

	# transmission_monitor_device_input['frequency points'] = num_eval_frequency_points
	...
	# fdtd[ 'x span' ] += device_size_lateral_um * 1e-6
	# fdtd[ 'y span' ] += device_size_lateral_um * 1e-6
	fdtd[ 'z max' ] = device_vertical_maximum_um * 1e-6 + 3e-6

	transmission_monitor_total_transmission = fdtd_hook.addpower()
	transmission_monitor_total_transmission['name'] = 'transmission_monitor_total_transmission'
	transmission_monitor_total_transmission['monitor type'] = '2D Z-normal'
	transmission_monitor_total_transmission['x'] = 0 * 1e-6
	transmission_monitor_total_transmission['x span'] = fdtd['x span']
	transmission_monitor_total_transmission['y'] = 0 * 1e-6
	transmission_monitor_total_transmission['y span'] = fdtd['y span']
	transmission_monitor_total_transmission['z'] = adjoint_vertical_um * 1e-6
	transmission_monitor_total_transmission['override global monitor settings'] = 1
	transmission_monitor_total_transmission['use wavelength spacing'] = 1
	transmission_monitor_total_transmission['use source limits'] = 1
	transmission_monitor_total_transmission['frequency points'] = num_design_frequency_points

	transmission_monitor_total_reflection = fdtd_hook.addpower()
	transmission_monitor_total_reflection['name'] = 'transmission_monitor_total_reflection'
	transmission_monitor_total_reflection['monitor type'] = '2D Z-normal'
	transmission_monitor_total_reflection['x'] = 0 * 1e-6
	transmission_monitor_total_reflection['x span'] = fdtd['x span']
	transmission_monitor_total_reflection['y'] = 0 * 1e-6
	transmission_monitor_total_reflection['y span'] = fdtd['y span']
	transmission_monitor_total_reflection['z'] = device_vertical_maximum_um * 1e-6 + 2e-6
	transmission_monitor_total_reflection['override global monitor settings'] = 1
	transmission_monitor_total_reflection['use wavelength spacing'] = 1
	transmission_monitor_total_reflection['use source limits'] = 1
	transmission_monitor_total_reflection['frequency points'] = num_design_frequency_points
	...
	eval_quadrant_transmission_data = np.zeros( ( 2, num_adjoint_sources, num_design_frequency_points ) )
	eval_focal_transmission_data = np.zeros( ( 2, num_design_frequency_points ) )
	eval_total_transmission_data = np.zeros( ( 2, num_design_frequency_points ) )
	eval_total_reflection_data = np.zeros( ( 2, num_design_frequency_points ) )
	for xy_idx in range(0, 2):
		fdtd_hook.load( job_names[ ( 'forward', xy_idx, dispersive_range_idx ) ] )

		# print( 'loaded ' + str( job_names[ ( 'forward', xy_idx, dispersive_range_idx ) ] ) )

		eval_focal_transmission_data[ xy_idx ] = get_transmission_magnitude( transmission_monitor_focal[ 'name' ] )
		eval_total_transmission_data[ xy_idx ] = get_transmission_magnitude( transmission_monitor_total_transmission[ 'name' ] )
		eval_total_reflection_data[ xy_idx ] = get_transmission_magnitude( transmission_monitor_total_reflection[ 'name' ] )
	...
	np.save( projects_directory_location + '/eval_total_transmission_data.npy', eval_total_transmission_data )
	np.save( projects_directory_location + '/eval_total_reflection_data.npy', eval_total_reflection_data )
	...
	transmission_monitor_total_transmission['use wavelength spacing'] = 1
	transmission_monitor_total_transmission['use source limits'] = 1
	transmission_monitor_total_transmission['frequency points'] = num_eval_frequency_points
	...
	transmission_monitor_total_reflection['use wavelength spacing'] = 1
	transmission_monitor_total_reflection['use source limits'] = 1
	transmission_monitor_total_reflection['frequency points'] = num_eval_frequency_points




	gaussian_eval_waist_radius_um = 0.5 * device_size_lateral_um

	gaussian_eval_sources = []
	for xy_idx in range(0, 2):
		gaussian_eval = fdtd_hook.addgaussian()
		gaussian_eval['name'] = 'gaussian_src_' + xy_names[xy_idx]
		gaussian_eval['angle phi'] = xy_phi_rotations[xy_idx]
		gaussian_eval['direction'] = 'Backward'
		gaussian_eval['x span'] = fdtd['x span']
		gaussian_eval['y span'] = fdtd['y span']
		gaussian_eval['z'] = ( device_vertical_maximum_um + 1 ) * 1e-6
		gaussian_eval['wavelength start'] = lambda_min_eval_um * 1e-6
		gaussian_eval['wavelength stop'] = lambda_max_eval_um * 1e-6

		gaussian_eval['waist radius w0'] = 0.5 * device_size_lateral_um * 1e-6

		if use_gaussian_sources:
			gaussian_eval['waist radius w0'] = gaussian_waist_radius_um * 1e-6


		gaussian_eval['distance from waist'] = -1e-6

		gaussian_eval['frequency dependent profile'] = 1
		gaussian_eval['number of field profile samples'] = 15

		gaussian_eval_sources.append(gaussian_eval)


	# job_names = {}
	# for xy_idx in range(0, 2):
	# 	disable_all_sources()
	# 	for gaussian_eval_src_idx in range( 0, len( gaussian_eval_sources ) ):
	# 		fdtd_hook.select( gaussian_eval_sources[gaussian_eval_src_idx]['name'] )
	# 		fdtd_hook.set( 'enabled', 0 )
	...
	# 		fdtd_hook.set( 'enabled', 1 )
	# 	else:
	# 		fdtd_hook.select( gaussian_eval_sources[xy_idx]['name'] )
	# 		fdtd_hook.set( 'enabled', 1 )

	# 	job_name = 'gaussian_eval_job_' + str( xy_idx ) + '.fsp'
	# 	fdtd_hook.save( projects_directory_location + "/optimization.fsp" )
	# 	job_names[ ( 'gaussian', xy_idx ) ] = add_job( job_name, jobs_queue )

	# run_jobs( jobs_queue )


	# total_gaussian_transmission = np.zeros( ( 2, num_eval_frequency_points ) )
	# total_gaussian_reflection = np.zeros( ( 2, num_eval_frequency_points ) )
	# total_focal_transmission = np.zeros( ( 2, num_eval_frequency_points ) )
	# eval_quadrant_transmission_gaussian_data = np.zeros( ( 2, num_adjoint_sources, num_eval_frequency_points ) )
	# focal_e_data = [ [], [] ]

	# for xy_idx in range(0, 2):
	# 	fdtd_hook.load( job_names[ ( 'gaussian', xy_idx ) ] )

	# 	total_gaussian_transmission[ xy_idx ] = get_transmission_magnitude( transmission_monitor_total_transmission[ 'name' ] )
	# 	if transmission_monitor_total_reflection[ 'z max' ] < fdtd[ 'z max' ]:
	# 		total_gaussian_reflection[ xy_idx ] = get_transmission_magnitude( transmission_monitor_total_reflection[ 'name' ] )
	# 	total_focal_transmission[ xy_idx ] = get_transmission_magnitude( transmission_monitor_focal[ 'name' ] )

	# 	for adj_src_idx in dispersive_range_to_adjoint_src_map[ dispersive_range_idx ]:					
	# 		eval_quadrant_transmission_gaussian_data[ xy_idx ][ adj_src_idx ] = get_transmission_magnitude( transmission_monitors[ adj_src_idx ][ 'name' ] )

	# 	focal_e_data[ xy_idx ].append( get_efield( transmission_monitor_focal[ 'name' ] ) )


	# np.save( projects_directory_location + '/focal_e_data_gaussian.npy', focal_e_data )

	# np.save( projects_directory_location + '/eval_quadrant_transmission_data.npy', eval_quadrant_transmission_data )
	# np.save( projects_directory_location + '/eval_focal_transmission_data.npy', eval_focal_transmission_data )
	# np.save( projects_directory_location + '/eval_quadrant_transmission_gaussian_data.npy', eval_quadrant_transmission_gaussian_data )


	# np.save( projects_directory_location + '/total_gaussian_transmission.npy', total_gaussian_transmission )
	# if transmission_monitor_total_reflection[ 'z max' ] < fdtd[ 'z max' ]:
	# 	np.save( projects_directory_location + '/total_gaussian_reflection.npy', total_gaussian_reflection )
	# np.save( projects_directory_location + '/total_focal_transmission.npy', total_focal_transmission )


	# transmission_gaussian_normalization = np.zeros( ( 2, num_eval_frequency_points ) )
	...
	# 	job_name = 'gaussian_eval_job_blank_' + str( xy_idx ) + '.fsp'
	# 	fdtd_hook.save( projects_directory_location + "/optimization.fsp" )
	# 	job_names[ ( 'gaussian_blank', xy_idx ) ] = add_job( job_name, jobs_queue )

	# run_jobs( jobs_queue )

	# for xy_idx in range(0, 2):
	# 	fdtd_hook.load( job_names[ ( 'gaussian_blank', xy_idx ) ] )

	# 	transmission_gaussian_normalization[ xy_idx ] = get_transmission_magnitude( transmission_monitor_device_input[ 'name' ] )


	# np.save( projects_directory_location + '/transmission_gaussian_normalization.npy', transmission_gaussian_normalization )
	...
	if pdaf_scan:
		...
		num_x_positions = 20#5#7
		num_y_positions = 1#5#7

		theta_angles = np.linspace( source_angle_theta_deg - 8, source_angle_theta_deg + 8, num_x_positions )# np.linspace( 0, pdaf_angular_range, num_x_positions )
		phi_angles = np.linspace( 0, 0., num_y_positions )# 360., num_y_positions )

		x_positions = ( 7. * 0.5 / 8. ) * np.linspace( -device_size_lateral_um, device_size_lateral_um, num_x_positions )
		y_positions = ( 7. * 0.5 / 8. ) * np.linspace( -device_size_lateral_um, device_size_lateral_um, num_y_positions )


		transmission_pdaf_pol_quad = np.zeros( ( num_x_positions, num_y_positions, 2, 4, num_eval_frequency_points ) )
		transmission_pdaf_defocus_pol_quad = np.zeros( ( num_defocus_points, 2, 4, num_eval_frequency_points ) )


		if pdaf_defocus_gaussian:

			for d_idx in range( 0, num_defocus_points ):

				disable_all_sources()

				for pol_idx in range( 0, 2 ):
					
					gaussian_eval_sources[ pol_idx ][ 'x span' ] = 2 * fdtd[ 'x span' ]
					gaussian_eval_sources[ pol_idx ][ 'y span' ] = 2 * fdtd[ 'y span' ]

					gaussian_eval_sources[ pol_idx ][ 'x' ] = 0 * 1e-6
					gaussian_eval_sources[ pol_idx ][ 'y' ] = 0 * 1e-6

					gaussian_eval_sources[ pol_idx ][ 'distance from waist' ] = ( -1 + pdaf_defocus_amounts_um[ d_idx ] ) * 1e-6


					for disable_pol_idx in range( 0, 2 ):
						fdtd_hook.select( gaussian_eval_sources[ disable_pol_idx ][ 'name' ] )
						fdtd_hook.set( 'enabled', 0 )

					fdtd_hook.select( gaussian_eval_sources[ pol_idx ][ 'name' ] )
					fdtd_hook.set( 'enabled', 1 )


					job_name = 'pdaf_eval_job_' + str( pol_idx ) + '_' + str( d_idx ) + '.fsp'
					fdtd_hook.save( projects_directory_location + "/optimization.fsp" )
					job_names[ ( 'pdaf_defocus', d_idx, pol_idx ) ] = add_job( job_name, jobs_queue )

			run_jobs( jobs_queue )

			for d_idx in range( 0, num_defocus_points ):
				fdtd_hook.load( job_names[ ( 'pdaf_defocus', d_idx, pol_idx ) ] )

				for quad_idx in range( 0, 4 ):
					transmission_pdaf_defocus_pol_quad[ d_idx, pol_idx, quad_idx, : ] = get_transmission_magnitude( transmission_monitors[ quad_idx ][ 'name' ] )

			np.save( projects_directory_location + '/transmission_pdaf_pol_quad_defocus.npy', transmission_pdaf_defocus_pol_quad )

		# else:


		for y_idx in range( 0, num_y_positions ):
			for x_idx in range( 0, num_x_positions ):
				...
				for pol_idx in range( 0, 2 ):
					...
					gaussian_eval_sources[ pol_idx ][ 'x' ] = x_positions[ x_idx ] * 1e-6
					gaussian_eval_sources[ pol_idx ][ 'y' ] = y_positions[ y_idx ] * 1e-6

					if pdaf_defocus_gaussian:
						gaussian_eval_sources[ pol_idx ][ 'distance from waist' ] = ( -1 + pdaf_gaussian_defocus_um ) * 1e-6

					if pdaf_angular:

						r = ( gaussian_eval_sources[ pol_idx ][ 'z' ] - design_import[ 'z max' ] ) * np.tan( np.pi * theta_angles[ x_idx ] / 180. )
						delta_x = r * np.cos( np.pi * phi_angles[ y_idx ] / 180.)
						delta_y = r * np.sin( np.pi * phi_angles[ y_idx ] / 180.)

						if use_gaussian_sources:
							forward_sources[ pol_idx ][ 'x span' ] = 2. * fdtd[ 'x span' ]
							forward_sources[ pol_idx ][ 'y span' ] = 2. * fdtd[ 'y span' ]

							forward_sources[ pol_idx ][ 'x' ] = delta_x
							forward_sources[ pol_idx ][ 'y' ] = delta_y

							forward_sources[ pol_idx ][ 'angle theta' ] = theta_angles[ x_idx ]
							forward_sources[ pol_idx ][ 'angle phi' ] = phi_angles[ y_idx ]

							forward_sources[ pol_idx ][ 'polarization angle' ] = -phi_angles[ y_idx ] + xy_phi_rotations[ pol_idx ]
						else:
							gaussian_eval_sources[ pol_idx ][ 'x' ] = delta_x
							gaussian_eval_sources[ pol_idx ][ 'y' ] = delta_y

							gaussian_eval_sources[ pol_idx ][ 'angle theta' ] = theta_angles[ x_idx ]
							gaussian_eval_sources[ pol_idx ][ 'angle phi' ] = phi_angles[ y_idx ]

							gaussian_eval_sources[ pol_idx ][ 'polarization angle' ] = -phi_angles[ y_idx ] + xy_phi_rotations[ pol_idx ]
					...
					job_name = 'pdaf_eval_job_pw_' + str( pol_idx ) + '_' + str( x_idx ) + '.fsp'
					fdtd_hook.save( projects_directory_location + "/optimization.fsp" )
					# full_name = projects_directory_location + "/" + job_name
					# job_names[ ( 'pdaf', x_idx, pol_idx ) ] = full_name
					job_names[ ( 'pdaf', x_idx, pol_idx ) ] = add_job( job_name, jobs_queue )
			...
			for x_idx in range( 0, num_x_positions ):
				for pol_idx in range( 0, 2 ):
					...
					fdtd_hook.load( job_names[ ( 'pdaf', x_idx, pol_idx ) ] )

					for quad_idx in range( 0, 4 ):
						transmission_pdaf_pol_quad[ x_idx, y_idx, pol_idx, quad_idx, : ] = get_transmission_magnitude( transmission_monitors[ quad_idx ][ 'name' ] )
			

					focal_p_field = get_Pfield( transmission_monitor_focal[ 'name' ] )
					if ( x_idx == 0 ) and ( pol_idx == 0 ):

						focal_power_data_shape = [ 2, num_x_positions, num_y_positions ] + list( focal_p_field.shape )
						focal_power_data = np.zeros( focal_power_data_shape, dtype=focal_p_field.dtype )

					focal_power_data[ pol_idx, x_idx, y_idx ] = focal_p_field



		if pdaf_defocus_gaussian:
			# np.save( projects_directory_location + '/transmission_pdaf_pol_quad_defocus_p4um.npy', transmission_pdaf_pol_quad )
			# np.save( projects_directory_location + '/transmission_pdaf_pol_quad_defocus_m4um.npy', transmission_pdaf_pol_quad )
			np.save( projects_directory_location + '/transmission_pdaf_pol_quad_defocus.npy', transmission_pdaf_defocus_pol_quad )
		elif pdaf_angular:
			np.save( projects_directory_location + '/transmission_pdaf_pol_quad_angular.npy', transmission_pdaf_pol_quad )
			np.save( projects_directory_location + '/focal_power_data_pw.npy', focal_power_data )
		# else:
		np.save( projects_directory_location + '/transmission_pdaf_pol_quad.npy', transmission_pdaf_pol_quad )
	...
	#
	# Fixing a certain geometry for quartering and gradient accumuluation, but a good place to start I think!
	#
				...
				for dispersive_range_idx in range( 0, num_dispersive_ranges ):
					...
					if add_pdaf:
						...
						for xy_idx in range(0, 2):
							...
							job_name = 'pdaf_forward_job_' + str( xy_idx ) + "_" + str( dispersive_range_idx ) + '.fsp'
							fdtd_hook.save( projects_directory_location + "/optimization.fsp" )
						...
						for xy_idx in range(0, 2):
							...
							job_name = 'pdaf_adjoint_job_' + str( xy_idx ) + '_' + str( dispersive_range_idx ) + '.fsp'
							fdtd_hook.save( projects_directory_location + "/optimization.fsp" )
			...
			if start_from_step == 0 or start_from_step == 2:
				...
				for xy_idx in range( 0, 2 ):
					for adj_src_idx in range( 0, 4 ):
						np.save( projects_directory_location + '/debug_quadrant_transmission_data_' + str( xy_idx ) + '_' + str( adj_src_idx ) + '.npy', quadrant_transmission_data[xy_names[xy_idx]][ adj_src_idx ] )
				...
				for focal_idx in range(0, num_focal_spots):             # For each focal spot:
					...
					for polarization_idx in range(0, len(polarizations)):
						...
						for spectral_idx in range(0, total_weighting.shape[0]):
							...
							transmission_addition = weight_spectral_rejection * get_quad_transmission_data[focal_idx][spectral_focal_plane_map[focal_idx][0] + spectral_idx] / one_over_weight_focal_plane_map_performance_weighting
							transmission_by_wavelength[ spectral_focal_plane_map[focal_idx][0] + spectral_idx ] += transmission_addition
				...
				if do_green_balance:
					weight_greens = ( 2. / len( weight_greens ) ) - ( stack_green_T**2 / np.sum( stack_green_T**2 ) )

					if np.min( weight_greens ) < 0:
						weight_greens -= np.min( weight_greens )
						weight_greens /= np.sum( weight_greens )

				log_file = open( projects_directory_location + "/log.txt", 'a' )
				log_file.write( "Green balance = " + str( stack_green_T ) + "\n" )
				log_file.write( "Green weights = " + str( weight_greens ) + "\n\n" )
				log_file.close()
				...
				print(np.shape(transmission_by_wl_evolution))
				print(np.shape(transmission_by_wavelength))
				...
				if add_pdaf:
					...
					np.save( projects_directory_location + "/pdaf_transmission.npy", pdaf_transmission_evolution )
						...
						for xy_idx in range(0, 2):
							...
							for spectral_idx in range(0, len( source_weight ) ):
								...
								gradient_component = np.sum(
									green_weight * (source_weight[spectral_idx] * gradient_performance_weight / total_weighting[spectral_idx]) *
