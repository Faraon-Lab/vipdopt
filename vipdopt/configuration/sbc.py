...
class SonyBayerConfig(Config):
    ...
    def _explicit_band_centering(self):
        ...
        weight_individual_wavelengths_by_quad = np.zeros((
            4,
        ...
        for quad_idx in range(4):
            ...
            for wl_idx in range(self.num_design_frequency_points):
                ...
                scaling_exp = -1.0 / np.log(0.5)
                weight_individual_wavelengths_by_quad[quad_idx, wl_idx] = np.exp(
                    -((wl_idx - band_center) ** 2) / (scaling_exp * choose_width**2)
                )
    ...
    def _add_infrared(self):
        ...
        spectral_focal_plane_map = [
            ...
            [
                4 + self.num_points_per_band,
                6 + self.num_points_per_band + (self.num_points_per_band // 2),
            ],
            [wl_idx_ir_start, (wl_idx_ir_start + 3)],
        ]

        ir_band_equalization_weights = [
            1.0 / (spectral_focal_plane_map[idx][1] - spectral_focal_plane_map[idx][0])
        ...
        for band_idx in range(len(spectral_focal_plane_map)):
            for wl_idx in range(
                spectral_focal_plane_map[band_idx][0],
                spectral_focal_plane_map[band_idx][1],
            ):
                weight_individual_wavelengths[wl_idx] = ir_band_equalization_weights[
                    band_idx
                ]
