...
class Pool:
    ...
    def __init__(self, executor: Executor, target: Callable, comm: MPI.Comm=None, *args) -> None:
        self.size = None
        self.queue: Deque[tuple[Future, Task]] = Deque()

        self.event = threading.Event()

        self.thread = threading.Thread(target=target, args=(self, executor._options, comm, *args))
        self.setup_threads()
...
def manager_function(pool: Pool, options: dict, intracomm: MPI.Intracomm=None, *args):
...
def manager_file(pool: Pool, options: dict, comm: MPI.Intracomm=None, *args):
    manager = FileManager()
...
class FileManager:
    ...
    def __init__(self):
        ...
        self.pending: dict[WorkerSet, tuple[Future, str, Task]] = dict()
        self.job_id = 0
    ...
    def _get_all_out_files(self) -> list[tuple[WorkerSet, str, Task]]:
        return [(key, outname, task) for key, (_, outname, task) in self.pending.items()]
    ...
    def read(self, idx: int):
        ...
        future, job_dir, task = self.pending.pop(key)
        assert os.path.exists(job_dir)

        logging.info(f'...Done executing \'mpirun -n {task.num_workers} {" ".join(task.args)}\'...\n')
