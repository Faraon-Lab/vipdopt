...
new_pattern = (
    np.copy(new_pattern).repeat(upsample_factor, axis=0).repeat(upsample_factor, axis=1)
)  # Upsample for accurate renderå


Nx = new_pattern.shape[0]
Ny = new_pattern.shape[1]
spatial_um = np.linspace(
    -0.5 * lattice_period_um * num_replications,
    0.5 * lattice_period_um * num_replications,
    num_replications * Nx,
)
...
for x_rep in range(num_replications):
    for y_rep in range(num_replications):
        ...
        if negative:
            new_pattern[x_start : (x_start + Nx), y_start : (y_start + Ny)] = np.array(
                ~np.greater(
                    binarize_density.repeat(upsample_factor, axis=0).repeat(
            ...
            new_pattern[x_start : (x_start + Nx), y_start : (y_start + Ny)] = (
                binarize_density.repeat(
...
contours, hierarchy = cv2.findContours(
    new_pattern, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
)
...
for idx in range(len(contours)):
    ...
    for pt_idx in range(len(contours[idx])):
        make_nd[pt_idx, :] = [
            1000.0 * spatial_um[contours[idx][pt_idx][0][0]],
            1000.0 * spatial_um[contours[idx][pt_idx][0][1]],
        ]
...
inverse_design.add(
    gdspy.CellArray(
        main_cell,
        num_cell_replications,
        num_cell_replications,
        (lattice_period_um, lattice_period_um),
        origin=(
            -0.5 * lattice_period_um * num_cell_replications,
            -0.5 * lattice_period_um * num_cell_replications,
        ),
    )
)
